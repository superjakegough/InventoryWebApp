...
export default class AppComponent extends Vue {
	...
	mounted() {
		this.themeColour(Number(getCookie('iwa-colour')));
		this.$vuetify.theme.error = "AF0E14";
		this.dark = getCookie('iwa-dark') == 'true' ? true : false;
	}

	themeDark() {
		this.dark = !this.dark;
		setCookie('iwa-dark', this.dark.toString());
	}

	themeColour(colour: number) {
		setCookie('iwa-colour', colour.toString());
		switch (colour) {
			case 0:
